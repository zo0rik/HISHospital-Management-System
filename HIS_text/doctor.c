...
static void addDoctor() {
    ...
    while (1) {
        scanf("%s", d.sex);
        if (strcmp(d.sex, "男性") != 0 && strcmp(d.sex, "女性")!= 0){
            printf("无效输入，请输入 '男性' 或 '女性': ");
        ...
        if (p->id == id) {
            ...
            for (int i = 0; i < cnt; i++) {
                ...
                case 4: printf("输入新性别: "); 
                    while (1) {
                        scanf("%s", p->sex);
                        if (strcmp(p->sex, "男性") != 0 && strcmp(p->sex, "女性") !=0 ){
                            printf("无效输入，请输入 '男性' 或 '女性': ");
...
static void queryDoctor() {
    int choice;
    printf("查询方式：1-按ID  2-按姓名（模糊）3-按职称: "); 
    if (scanf("%d", &choice) != 1)
		choice = -1; // 处理非数字输入
