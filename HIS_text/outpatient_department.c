...
void callPatient(const char* docId) {
    ...
    /* 【修复1】从scheduleList->next开始，跳过哨兵节点
       【修复2】直接用docId比较，不再做docId+1截断 */
    ...
    while (s) {
        if (strcmp(s->doctor_id, docId) == 0) {
            ...
            if (!duplicate && sCount < 20) { strcpy(availableDates[sCount], s->date); sCount++; }
        }
        s = s->next;
    }
    ...
    /* 后续代码保持不变，但注意候诊列表搜索中提取排号的部分需要兼容 "队列号:" 格式 */
    printf("\n========== [%s] 候诊大厅鱼骨图 ==========\n", targetDate);
