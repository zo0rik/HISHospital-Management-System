...
void changePassword(void) {
    ...
    printf("请输入旧密码 (输入-1取消): "); safeGetString(old, 50); /* 【规则C】0→-1 */
    if (strcmp(old, "-1") == 0) return; /* 【规则C】 */
    if (strcmp(old, admin.password) != 0) { printf("  [!] 旧密码错误！\n"); system("pause"); return; }

    printf("请输入新密码 (至少6位，仅限字母或数字): "); safeGetPassword(new1, 50); /* 【规则D】加至少6位 */
    if (strcmp(new1, "-1") == 0) return; /* 【规则C】 */
...
void editPersonalInfo(void) {
    ...
    printf("\n请输入新用户名 (直接输入-1保留原值): "); /* 【规则C】 */
    safeGetString(buffer, 50);
    if (strcmp(buffer, "-1") != 0 && strlen(buffer) > 0) strcpy(admin.username, buffer); /* 【规则C】 */

    printf("请输入新手机号 (直接输入-1保留原值): "); /* 【规则C】 */
    safeGetString(buffer, 20);
    if (strcmp(buffer, "-1") != 0 && strlen(buffer) > 0) strcpy(admin.phone, buffer); /* 【规则C】 */

    printf("请输入新邮箱 (直接输入-1保留原值): "); /* 【规则C】 */
    safeGetString(buffer, 50);
    if (strcmp(buffer, "-1") != 0 && strlen(buffer) > 0) strcpy(admin.email, buffer); /* 【规则C】 */
...
void personalMenu() {
    ...
    do {
        ...
        while (1) {
            choice = safeGetInt();
            if (choice == 1 || choice == 2 || choice == -1) break; /* 【规则B】0→-1 */
...
void adminMenu(void) {
    ...
    do {
        ...
        while (1) {
            choice = safeGetInt();
            if (choice == -1 || (choice >= 1 && choice <= 5)) break; /* 【规则B】 */
            printf("  [!] 输入格式不合法，请正确输入菜单中提供的数字编号！\n请重新下达管理指令: ");
        }
        ...
        case 3:
            ...
            while (1) {
                sub = safeGetInt();
                if (sub == 1 || sub == 2 || sub == -1) break; /* 【规则B】0→-1 */
                printf("  [!] 输入格式不合法，请正确输入菜单中提供的数字编号！\n请重新选择: ");
            }
            if (sub == 1) doctorMenu();
